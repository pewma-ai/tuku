...
Los janitors viven en `src/jntr/`, un módulo por cada `jntr.<algo>`. El primero es `jntr.entrada_insertar`, que consume `002-001`. `cadena.py` todavía exige que el paso previo se haya corrido en la misma sesión de pytest; reproducir la cadena entera desde el fixture del epic queda pendiente.
