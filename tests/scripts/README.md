...
| Módulo | Qué ofrece |
| --- | --- |
| `vault.py` | Dueño de los dos playgrounds: `BANCO` (en `/tmp`) es donde se corre y `ARCHIVO` (`playground/` del repo) donde el autor analiza, y `archivar` copia del uno al otro al terminar. `exigir_banco_limpio` falla si algo heredable (`AGENTS.md`, `CLAUDE.md`, `.git`) cuelga por encima del banco. `preparar_dir` da la carpeta de un escenario, e `instantanea` y `delta` comparan dos estados, que es lo que afirma un escenario encadenado. Más `diff_recursivo` contra el template en vivo, `ahora_sembrado` para derivar el esperado sin congelarlo, y `placeholders_sin_sustituir` para que un placeholder nuevo en el template no pase en silencio |
| `gherkin.py` | `correr` ejecuta un escenario leyendo su `.md`: parsea los bloques `bash` de `Dado`, `Cuando` y `Entonces` y los corre con el directorio de trabajo en `<banco>/<slug>/` y `TUKU_HOME` en el checkout. El `.md` es la fuente ejecutable y el arnés queda con las aserciones; la convención está en [`../escenarios/README.md`](../escenarios/README.md) |
