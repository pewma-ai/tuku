...
`001-002` cubre la siembra byte a byte sin nombre. Este es la única cobertura del parámetro `autor` de `init()`.
...
Fecha fija 2026-08-11, la misma de `001-002`, para que el vault sea comparable con `diff -r` (salvo `LIBRO-DE-ESTILO.md`).

## Cómo se corre

```bash
uv run pytest tests/escenarios/ -k 001_004
```
