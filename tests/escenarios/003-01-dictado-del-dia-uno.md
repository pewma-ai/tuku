# Escenario · 003-01-dictado-del-dia-uno

**Cubre:** epic 003, fase 1, la parte con LLM. Único escenario del epic que gasta tokens, y dueño de `registros.md`, el fixture que los otros nueve consumen.
...
Así que se invierte. La salida del agente se congela en `registros.md`, la cadena determinista consume ese texto, y este escenario verifica aparte que el agente vivo lo reproduce. El LLM queda en los dos extremos y el medio es determinista.

## Estado inicial

Dos, y no uno:

- Para la verificación del agente, el fixture `vacio`, igual que [`002-01`](002-01-registro-en-su-dia.md): tiene que trabajar sobre el mismo vault que tenía el autor al dictar.
- Para la revisión a mano, el estado final que dejó [`003-02`](003-02-propuesta-no-escribe.md), que es el día uno completo.

## El fixture

```text
fixtures/003-01-dictado-del-dia-uno/
  dictado.md     # lo que el autor dijo, generado desde el corpus
  registros.md    # las líneas de bitácora que produjo, congeladas
```
...
`registros.md` se congela: es salida de agente y no tiene original vivo contra el cual compararse.

## Escenario: el dictado produce los registros congelados
...
Entonces cada línea que produce coincide con `registros.md` en su **ontología cerrada**: la marca, su posición y el ámbito
...
Y lo que el hecho sugiere quedó como propuesta, tal como afirma [`003-02`](003-02-propuesta-no-escribe.md)
...
- **Que cada registro se sostenga solo.** Deícticos resueltos, personas con su rol la primera vez, tiempo relativo convertido en fecha. La prueba es leerlo en tres años.
- **El desglose.** Si el corte en hechos fue el que haría una persona, o el agente cortó por frase.
- **Cuánto se desvió de `registros.md`** y en qué. Una diferencia de redacción es información sobre la spec; una diferencia de consecuencia es un defecto.
