# Escenario · 001-005-init-no-toca-la-red
...
## Por qué existe, si `001-002` ya siembra sin red

`001-002` no ve tráfico, pero no lo impide: si mañana `init()` (o algo que importe) intentara resolver un nombre o abrir una conexión, `001-002` seguiría pasando. Este lo fuerza: con `socket.socket`, `socket.create_connection` y `socket.getaddrinfo` parchados para lanzar `RedBloqueada`, cualquier intento sube y el test falla señalando exactamente eso.
...
```bash
uv run pytest tests/escenarios/ -k 001_005
```

Deja el vault en `playground/001-005-init-no-toca-la-red/`.

## Qué se mira a mano

- Nada específico: si el test pasa, la afirmación "offline" está aislada. La revisión byte a byte del vault la hace `001-002`.
