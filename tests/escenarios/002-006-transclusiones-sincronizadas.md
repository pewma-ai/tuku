...
Las dos inyecciones entran por la **segunda vía**: no son hechos de la vida del autor, así que no pasan por la bitácora. Se rompe el archivo a mano y se invoca el janitor, la única otra puerta ([`spec/flujo-informacion.md`](../../spec/flujo-informacion.md)).
...
Esta se ve sola, es una caja de error en el día. El janitor la quita igual: una caja de error en `AHORA.md` es un defecto, no un estado válido.
...
- Abrirlo en Obsidian **entre las dos inyecciones**, con la transclusión rota, y confirmar que la falla silenciosa no se ve. Es lo que justifica el janitor.
- Que el estado final sea indistinguible del que dejó `002-005`, con `diff -r` contra su playground.
