# Escenario · 001-002-init-siembra-el-estado-cero
...
Dado un directorio vacío y el árbol de TUKU a la vista (`home` = el checkout)
Cuando se llama a `init(destino, variante="vanilla", desde=2026-08-11, home=RAIZ)`
...
Es la verificación byte a byte que antes hacía `test_001_001`, ahora sobre `init()` en vez de `install.sh`. Llama a la función importable con `home=RAIZ`: la decisión 4 del epic 002 reserva ese camino para todos los `001-00X` menos el `001-001`.
...
La tercera afirmación cubre el crecimiento del template: si un archivo nuevo trae `DD de mes` y `init()` no lo sustituye, las otras dos pasan en silencio y esta no.
...
```bash
uv run pytest tests/escenarios/ -k 001_002
```

Deja el vault en `playground/001-002-init-siembra-el-estado-cero/`, a la vista.
