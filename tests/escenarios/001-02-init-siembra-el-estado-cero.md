...
Es la verificación byte a byte que antes hacía `test_001_01`, ahora sobre `init()` en vez de `install.sh`. Llama a la función importable con `home=RAIZ`: la decisión 4 del epic 002 reserva ese camino para todos los `001-00X` menos el `001-01`.
...
Deja el vault en `playground/001-02-init-siembra-el-estado-cero/`, a la vista.
