...
Ninguno de los dos es un archivo: son la salida de un janitor que se ejecuta en ese momento (ver `README.md`, árbol de directorios). Quedan en **caché de sesión** y no se releen en cada turno. Una sesión que no va a escribir bitácora no necesita ninguno de los dos.

Si el arnés no sabe ejecutar comandos y solo lee archivos, hay que materializarlos, y ahí reaparece el problema de que envejecen. Eso es limitación del arnés, no del diseño, y por eso vive en esta spec y no en `flujo-informacion.md`.

## Carga diferida de reglas

Las reglas de cada consecuencia no viajan en el contexto base. Se abre `reglas/<consecuencia>.tuku.md` solo cuando el paso 3 del flujo (ver `flujo-informacion.md`) detectó que esa consecuencia aplica. Una entrada sin consecuencias termina en el paso 4 sin haber cargado nada extra.
