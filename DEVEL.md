...
- **Tres capas, y `cli.py` es solo la de arriba.** El núcleo son funciones puras de texto a texto, sin disco, y es donde vive la lógica: `entry.add`, `todo.abrir`, `note.lint`. Encima, un caso de uso por comando en el módulo de su noun (`entry.add_al_vault`, `todo.cerrar_en_vault`, `note.crear_con_constancia`), que lee, llama al núcleo, escribe y devuelve un [`Resultado`](src/tuku/resultado.py). Arriba del todo, [`cli.py`](src/tuku/cli.py): argparse y la traducción de ese `Resultado` a texto y código de salida, nada más. Una regla del sistema que termine dentro de un `_cmd_*` está en la capa equivocada, y la señal es que solo se puede probar con un escenario E2E.
