...
## 3.4 Canales y el agente

El perfil admite varias puertas sin que ninguna sea la verdadera, porque todas leen y escriben el mismo repositorio de texto. Sentado frente al computador, la puerta es **Obsidian**, local, sobre la carpeta del usuario. Servido por web, los mismos archivos se publican con **Quartz**. En el bolsillo está **Telegram**. Las tres son superficie; ninguna es TUKU.

La conversación la sostiene **Hermes**, un agente preconfigurado que llega sabiendo las reglas de TUKU y leyendo el repositorio del usuario. Es el mismo agente detrás de las tres puertas, no tres implementaciones distintas.

Hermes no calcula: escribe lo que el usuario le cuenta y deja que los **janitors** del comando `tuku` hagan lo determinista —las derivaciones del §4, las proyecciones del §3, la higiene del repositorio—. Esta división es la que permite reemplazar el agente sin perder nada, porque lo que el sistema garantiza no depende de él (`brief.md` §4.2, §4.4).
...
|---|---|---|
| Apertura | `plan_FECHA_tipo.md` | propuesto, luego del humano |
| Cierre | `resultados_FECHA_tipo.md` | propuesto, luego del humano |
...
reporte parte de un filtro determinista que el agente **redacta**, no de una inferencia que
...
la razón por la que el alcance del reporte se define por entidad y no por filtro plano.

**El reporte es la memoria de largo plazo.** Markdown no es una base de datos: la consulta
histórica se responde por reportes, no escaneando el detalle. El crudo se conserva por año
y no se destruye nunca, pero un reporte pobre es memoria perdida. Por eso su formato es un
